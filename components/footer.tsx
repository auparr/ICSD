import * as React from "react"; // FIX: missing — React.ReactNode etc. undefined without this
...
// FIX: was inside component — `new Date().getFullYear()` and all arrays recreated every render
...
// ─── Component ────────────────────────────────────────────────────────────────
// No "use client" needed — pure static markup, runs as Server Component
    ...
    <footer className="bg-gradient-to-br from-[#2E2E2E] to-[#1A1A1A] text-white">
      <div className="container mx-auto px-4 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12 mb-12">
          ...
          <div className="lg:col-span-2">
            ...
            <address className="not-italic space-y-3">
              ...
              <div className="flex items-center gap-3">
                <Phone
                  className="w-5 h-5 text-[#6BB9E0] flex-shrink-0"
                  aria-hidden="true"
                />
                <a
              ...
              <div className="flex items-center gap-3">
                <Mail
                  className="w-5 h-5 text-[#F4C27F] flex-shrink-0"
                  aria-hidden="true"
                />
                <a
                  href="mailto:icsdsalam@gmail.com"
                  className="text-white/70 text-sm hover:text-white transition-colors"
                >
                  icsdsalam@gmail.com
                </a>
          ...
          {/* FIX: key={index} → key={link.label} — index keys break reconciliation when list order changes */}
          {(["tentang", "program", "informasi"] as const).map((section) => (
            <div key={section}>
              <h4 className="font-bold text-lg mb-4 capitalize">{section}</h4>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-6">
            ...
            <div className="flex gap-4">
              {SOCIAL_MEDIA.map(({ icon: Icon, href, label }) => (
                <a
                  key={label} // FIX: key={index} → key={label}
                  href={href}
                  aria-label={label}
            ...
            <p className="text-white/50 text-sm text-center">
              © {CURRENT_YEAR} Salam ICSD. Semua hak dilindungi.
            </p>
