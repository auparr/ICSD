...
const STAR_ARRAYS = TESTIMONIALS.map((t) => Array.from({ length: t.rating }, (_, i) => i));
...
export function TestimonialsSection() {
  ...
  const prev = useCallback(() => setCurrentIndex((i) => (i - 1 + TOTAL) % TOTAL), []);
  ...
  return (
    ...
    >
      {/* Decorative blobs */}
      <div aria-hidden="true" className="absolute top-10 left-10 w-32 h-32 bg-[#5BAA6A]/10 rounded-full blur-3xl" />
      <div aria-hidden="true" className="absolute bottom-10 right-10 w-40 h-40 bg-[#6BB9E0]/10 rounded-full blur-3xl" />
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-white rounded-3xl shadow-2xl p-8 md:p-12 relative">
            ...
            <div className="grid md:grid-cols-[auto,1fr] gap-8 items-center">
              ...
              <div className="flex flex-col items-center md:items-start">
                <div className="relative w-24 h-24 rounded-full overflow-hidden shadow-lg mb-4">
                  <Image src={current.image} alt={`Foto ${current.name}`} fill sizes="96px" className="object-cover" />
                </div>
                {/* BUG FIX: was spreading Array(n) which is sparse — Array.from is correct */}
                <div className="flex gap-1 mb-2" aria-label={`Rating: ${current.rating} bintang`}>
                  {STAR_ARRAYS[currentIndex].map((i) => (
                    <Star key={i} className="w-5 h-5 fill-[#F4C27F] text-[#F4C27F]" aria-hidden="true" />
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div>
                  <h4 className="text-xl font-bold text-[#2E2E2E]">{current.name}</h4>
            ...
            {/* Nav buttons */}
            <div className="flex justify-center gap-4 mt-8" role="group" aria-label="Navigasi testimoni">
              <button
                onClick={prev}
            ...
            {/* Dots */}
            <div className="flex justify-center gap-2 mt-6" role="tablist" aria-label="Pilih testimoni">
              {TESTIMONIALS.map((t, index) => (
                <button
                  ...
                  className={`h-2 rounded-full transition-all ${
                    index === currentIndex ? "bg-[#5BAA6A] w-8" : "bg-[#5BAA6A]/30 w-2"
                  }`}
