...
import type { Variants } from "framer-motion"; // FIX: import Variants type
...
// ─── Static path data outside component ──────────────────────────────────────
// IMPORTANT: All paths within each variant MUST have the same number of
// commands and points — framer-motion morphs between them numerically.
// Mismatched node counts cause broken/jumping animations.
const PATH_SETS: Record<
  NonNullable<CurvedDividerProps["variant"]>,
  string[]
> = {
  wave: [
...
// FIX: transition config extracted as const — not recreated on every render
const PATH_TRANSITION = {
  duration: 8,
  ease: "easeInOut",
  repeat: Infinity,
  repeatType: "mirror" as const, // FIX: was inferred as `string`, needs literal type for framer-motion
} satisfies Parameters<typeof motion.path>[0]["transition"];
